...
async def parse_target_and_args(message: Message) -> Tuple[Optional[int], Optional[sqlite3.Row], Optional[str]]:
    ...
    if message.reply_message:
        ...
        target_admin = db.get_admin_by_id(target_id, chat_id) if chat_id >= 2000000000 else None
...
async def check_permission(message: Message, command_name: str) -> bool:
    ...
    # В личных сообщениях нет системы администраторов чата
...
# Исправленные Middleware
class MuteCheckMiddleware(BaseMiddleware):
    async def pre(self, message: Message):
        # Пропускаем сообщения не из чатов
        if message.from_id < 0 or message.peer_id < 2000000000:
            return
...
class MessageLoggingMiddleware(BaseMiddleware):
    async def pre(self, message: Message):
        if message.from_id > 0 and message.peer_id:
            try: 
                db.add_message(message.from_id, message.peer_id, datetime.now())
            except Exception as e: 
...
# Система запросов (комментируем, так как нужно импортировать PayloadContainsRule)
...
async def help_cmd(message: Message):
    ...
    # Разные help для ЛС и чатов
    if message.peer_id < 2000000000:
        help_text = f"""{EMOJI['help']} Команды бота в личных сообщениях {EMOJI['help']}
...
{EMOJI['info']} Для работы в чатах добавьте бота в беседу и выдайте права администратора."""
    else:
...
async def test_cmd(message: Message): 
    await message.answer(f"{EMOJI['success']} Бот работает! Peer ID: {message.peer_id}")
...
async def admins_cmd(message: Message):
    ...
    if message.peer_id < 2000000000:
        return await message.answer(f"{EMOJI['info']} Эта команда доступна только в чатах.")
...
async def internal_punishment_handler(issuer_id: int, peer_id: int, message: Message, cmd: str, is_add: bool):
    if peer_id < 2000000000:
        await bot.api.messages.send(peer_id=peer_id, message=f"{EMOJI['info']} Эта команда доступна только в чатах.", random_id=0)
        return
    ...
    if not issuer:
        return await bot.api.messages.send(peer_id=peer_id, message=f"{EMOJI['error']} Не удалось определить ваши права администратора.", random_id=0)
...
async def unmute_cmd(message: Message, text: Optional[str] = None):
    ...
    if not issuer:
        return await message.answer(f"{EMOJI['error']} Не удалось определить ваши права администратора.")
    muted_by_admin = db.get_admin_by_id(mute_info['muted_by_id'], message.peer_id)
    if muted_by_admin and muted_by_admin['level'] >= issuer['level'] and muted_by_admin['user_id'] != issuer['user_id']: 
        return await message.answer(f"{EMOJI['error']} Нельзя снять мут от админа с равным/большим уровнем ({muted_by_admin['nickname']})!")
    db.remove_mute(target_id, message.peer_id)
...
async def show_user_logs(message: Message, user_id: int, chat_id: int):
    target_admin = db.get_admin_by_id(user_id, chat_id) if chat_id >= 2000000000 else None
    if not target_admin:
...
async def msgcount_cmd(message: Message, text: Optional[str] = None):
    ...
    if not target_admin:
        ...
        target_admin = db.get_admin_by_id(message.from_id, message.peer_id)
        args_text = ""
...
async def godmode_cmd(message: Message, args: Optional[str] = None):
    ...
    if not db.get_admin_by_id(target_id, message.peer_id): 
        db.add_admin(target_id, message.peer_id, nickname, message.from_id, 9, "Владелец")
    else: 
        db.update_admin(target_id, message.peer_id, 'level', 9)
        db.update_admin(target_id, message.peer_id, 'position', "Владелец")
...
async def dev_mode_cmd(message: Message, mode: Optional[str] = None):
    ...
    dev_admin_local = db.get_admin_by_id(DEV_USER_ID, message.peer_id) if message.peer_id >= 2000000000 else None
    ...
    if mode.lower() == "on":
        ...
        if message.peer_id >= 2000000000 and dev_admin_local:
            db.update_admin(DEV_USER_ID, message.peer_id, "level", 1)
...
async def daily_cmd(message: Message):
    user_data = db.get_user_global_data(message.from_id)
    if not user_data:
        db.update_balance(message.from_id, 0)
        user_data = db.get_user_global_data(message.from_id)
...
async def pay_cmd(message: Message, text: Optional[str] = None):
    ...
    db.update_balance(message.from_id, -amount)
...
def register_requestable_commands():
    ...
    COMMAND_HANDLERS["mute"] = mute_cmd
    COMMAND_HANDLERS["unmute"] = unmute_cmd
    COMMAND_HANDLERS["kick"] = kick_cmd
    COMMAND_HANDLERS["setlvl"] = setlvl_cmd
    COMMAND_HANDLERS["bladd"] = blacklist_add_cmd
    COMMAND_HANDLERS["blrem"] = blacklist_remove_cmd
